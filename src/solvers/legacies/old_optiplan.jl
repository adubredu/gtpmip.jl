...
function optiplan_solve(domain_name, problem_name; max_levels=10)
    ...
    for t=1:T-1 
        ...
        fidx,_ = get_fluent_range_indices(graph, t+1, max_nf)
        # Eq. 5
        at = t 
        for f = 1:length(graph.props[t]) 
            ...
            if !isempty(act_inds)
                @constraint(model, sum([y[i,at] for i in act_inds]) >= x[f, fidx[2]+ad])
                # Eq. 6
                [@constraint(model, y[i,at]  <= x[f, fidx[2]+ad]) for i in act_inds]
            end
        ...
        # for f = 1:length(graph.props[t]) 
            ...
            if !isempty(act_inds)
                @constraint(model, sum([y[i,at] for i in act_inds]) >= x[f, fidx[2]+dl])
                #Eq. 8
                [@constraint(model, y[i,at]  <= x[f, fidx[2]+dl]) for i in act_inds]
            end
        ...
        # for f = 1:length(graph.props[t]) 
            ...
            if !isempty(act_inds)
                @constraint(model, sum([y[i,at] for i in act_inds]) >= x[f, fidx[2]+pa])
                #Eq. 10
                [@constraint(model, y[i,at]  <= x[f, fidx[2]+pa]) for i in act_inds]
            end
        ...
        # for f = 1:length(graph.props[t]) 
            ...
            if !isempty(act_inds)
                @constraint(model, sum([y[i,at] for i in act_inds]) == x[f, fidx[2]+pd]) 
            end
            # Eq. 12
            ...
            # Eq. 14
            fidx_prev,_ = get_fluent_range_indices(graph, t, max_nf)
            ...
            if t < T-1
            num_actions = length([act for act in graph.acts[t+1] if act.name != :NoOp])
            @constraint(model, sum(y[1:num_actions, t+1]) == 1)
            end
        end
